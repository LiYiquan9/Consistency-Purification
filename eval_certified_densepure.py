...
from runners.edm_onestep_cifar10 import EDM_onestep_cifar10
from runners.edm_multistep_cifar10 import EDM_multistep_cifar10
from runners.cm_cifar10 import CM_cifar10
...
class DensePure_Certify(nn.Module):
    def __init__(self, args, config):
        ...
        elif args.diffusion_type == 'edm_onesep':
            self.runner = EDM_onestep_cifar10(sigma=self.args.sigma, checkpoint_dir=self.args.checkpoint_dir, checkpoint_steps=self.args.checkpoint_steps)
        elif args.diffusion_type == 'edm_multistep':
            self.runner = EDM_multistep_cifar10(sigma=self.args.sigma, checkpoint_dir=self.args.checkpoint_dir, checkpoint_steps=self.args.checkpoint_steps)
        elif args.diffusion_type == 'cm':
            self.runner = CM_cifar10(sigma=self.args.sigma, checkpoint_dir=self.args.checkpoint_dir, checkpoint_steps=self.args.checkpoint_steps)
        else:
...
def parse_args_and_config():
    ...
    parser.add_argument('--num_t_steps', type=int, default=25, help='numbers of reverse t steps')
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='', help="path to model checkpoint")
    parser.add_argument('--checkpoint_steps', type=int, default=40, help="steps of model checkpoint")
