...
def get_sampling_fn(config, sde, model, shape, eps=1e-3, img=None, sigma=0.25):
    ...
    elif sampler_name.lower() == "edm_onestep_purification":
        sampling_fn = get_edm_onestep_purification_sampler(
            sde=sde,
            model=model,
            shape=shape,
            denoise=config.sampling.denoise,
    ...
    elif sampler_name.lower() == "edm_multistep_purification":
        sampling_fn = get_edm_multistep_purification_sampler(
            sde=sde,
            model=model,
            shape=shape,
            denoise=config.sampling.denoise,
    ...
    elif sampler_name.lower() == "cm_purification":
        sampling_fn = get_cm_purification_sampler(
            config=config,
            sde=sde,
            model=model,
            shape=shape,
...
def get_heun_sampler(sde, model, shape, denoise=True):
    def heun_sampler(rng, state):
        ...
        def loop_body(i, val):
            ...
            print("vec shape is ", vec_t.shape)
...
def get_edm_onestep_purification_sampler(sde, model, shape, denoise=True, img=None, sigma_=0.25):
    def heun_sampler(rng, state):
        denoiser_fn = mutils.get_denoiser_fn(
            sde, model, state.params_ema, state.model_state, train=False
        )
        ...
        def loop_body(i, t_pos):
            condition_met = (timesteps[i] >= sigma) & (timesteps[i + 1] < sigma)
        
            new_t_pos = lax.cond(
                condition_met,
                lambda _: i + (timesteps[i] - sigma >= sigma - timesteps[i + 1]),
                lambda _: t_pos,
                operand=None
            )
        ...
        t_pos = lax.fori_loop(0, num_timesteps - 1, loop_body, num_timesteps - 1)
...
def get_edm_multistep_purification_sampler(sde, model, shape, denoise=True, img=None, sigma_ = 0.25):
    def heun_sampler(rng, state):
        ...
        x = jax.lax.fori_loop(t_pos, sde.N - 1, loop_body, x)
...
def get_cm_purification_sampler(config, sde, model, shape, img=None, sigma_ = 0.25):
    
    # TODO: 1. float time 2. integer time 3. add noise + integer time
    ...
    def sampler(rng, state):
        ...
        t_pos = lax.fori_loop(0, num_timesteps - 1, loop_body, num_timesteps - 1)
        #jax.debug.print("timesteps are {}", timesteps)
        #jax.debug.print("t_pos is {}", t_pos)
        #jax.debug.print("t is {}", timesteps[t_pos])
        ...
        samples = model_fn(x, jnp.ones((x.shape[0],)) * timesteps[t_pos])
        return samples, 1
