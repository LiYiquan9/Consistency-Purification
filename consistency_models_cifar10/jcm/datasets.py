        ...
        def collate_fn_1(batch):
            ...
            if image_tensor.shape[0] == int(np.prod(batch_dims)):
                ...
                return {
                    ...
                    "eval": torch.full(batch_dims, False, dtype=torch.bool),
                }
            # If the batch size is not a multiple of the batch dimension, pad the batch with zeros.
                ...
                return {"image": padded_image, "label": padded_label, "mask": mask, "eval": torch.full(batch_dims, False, dtype=torch.bool)}
        ...
        def collate_fn_2(batch):
            ...
            if image_tensor.shape[0] == int(np.prod(batch_dims)):
                ...
                return {
                    ...
                    "label": label_tensor,
                    "mask": torch.ones_like(label_tensor),
                    "eval": torch.full(batch_dims, True, dtype=torch.bool),
                }
            # If the batch size is not a multiple of the batch dimension, pad the batch with zeros.
            else:
                pad_size = int(np.prod(batch_dims)) - image_tensor.shape[0]
                ...
                padded_label = padded_label.reshape(batch_dims)
                mask = mask.reshape(batch_dims)
                return {"image": padded_image, "label": padded_label, "mask": mask, "eval": torch.full(batch_dims, True, dtype=torch.bool)}
