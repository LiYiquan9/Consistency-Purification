...
def get_loss_fn(config, sde, score_model, state, rng, classifier_train_step_fn=None, classifier_eval_step_fn=None, guidance_model=None):
    ...
    if config.training.loss.lower() in ["dsm", "ssm"]:
        ...
        if config.training.loss.lower().startswith("continuous"):
            ...
            train_loss_fn = get_consistency_loss_fn(
                ...
                lpips_model=lpips_model,
                lpips_params=lpips_params,
                classifier_step_fn=classifier_train_step_fn,
            ...
            eval_loss_fn = get_consistency_loss_fn(
                ...
                lpips_model=lpips_model,
                lpips_params=lpips_params,
                classifier_step_fn=classifier_eval_step_fn,
                guidance_model=guidance_model
            )
...
def get_consistency_loss_fn(
    ...
    def loss_fn(rng, params, states, batch, target_params=None, num_scales=None):
        ...
        indices = jax.random.randint(next(rng), (x.shape[0],), 0, 3)
        choices_0 = jnp.array([0.5, 1.0, 2.0])

        t_0 = choices_0[indices]

        indices_arrange = jnp.arange(0, num_scales)
        t_arrange = (sde.t_max ** (1 / sde.rho) + indices_arrange / (num_scales - 1) * (sde.t_min ** (1 / sde.rho) - sde.t_max ** (1 / sde.rho)))**sde.rho
        
        def find_closest(choice, t_arrange):
            idx = jnp.argmin(jnp.abs(t_arrange - choice))
            return t_arrange[idx]

        choices = jax.vmap(lambda choice: find_closest(choice, t_arrange))(choices_0)
        t = choices[indices]

        z1 = jax.random.normal(next(rng), x.shape)
        
        x_t_1 = x + batch_mul(t_0, z1)
        
        dropout_rng = next(rng)
        Ft, new_states = mutils.get_distiller_fn(
            sde, model, params, states, train=train, return_state=True
        )(x_t_1, t, rng=dropout_rng if train else None)
...
def get_step_fn(
    ...
    def step_fn(carry_state, batch):
        ...
        (rng, state, classifier_eval) = carry_state
        ...
        new_carry_state = (rng, new_state,classifier_eval)
