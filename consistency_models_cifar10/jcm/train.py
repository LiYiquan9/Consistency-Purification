...
# # Initialize classifier guide trainer model
# example_img = jnp.ones((16, 32, 32, 3))
# classifier_trainer = ClassifierTrainer(exmp_imgs=parallel_(random.PRNGKey(0),
#                                                     example_img),
#                         lr=5e-4,
#                         weight_decay=1e-4,
#                         save_freq =config.training.snapshot_freq)

# classifier_trainer.load_model()
# classifier_train_step_fn = classifier_trainer.get_classifier_train_step_fn()
# classifier_eval_step_fn = classifier_trainer.get_classifier_eval_step_fn()


# # Build one-step training and evaluation functions
# train_loss_fn, eval_loss_fn, state = losses.get_loss_fn(
#     config, sde, score_model, state, next(rng), classifier_train_step_fn, classifier_eval_step_fn
# )
...
# from jcm.models.classifier_joint_training import ClassifierTrainer, parallel_
...
def train(config, workdir):
    ...
    # vit_model = FlaxViTModel.from_pretrained('google/vit-base-patch16-224-in21k')
    # from classifier.train_classifier import ResNet, ResNetBlock, WideResNet, guidance_classifier
    # from flax import linen as nn
    # guidance_model = guidance_classifier(
    #                                     model_name="WideResNet",
    #                                     model_class=WideResNet,
    #                                     model_hparams={"num_classes": 10},
    #                                     optimizer_name="SGD",
    #                                     optimizer_hparams={"lr": 0.01, "momentum": 0.9, "weight_decay": 1e-4},
    #                                     exmp_imgs=jax.device_put(jnp.ones((100, 32, 32, 3))),
    #                                     num_epochs=100
    #                                 )

    # Build one-step training and evaluation functions
    train_loss_fn, eval_loss_fn, state = losses.get_loss_fn(
        config, sde, score_model, state, next(rng), classifier_train_step_fn=classifier_train_step_fn, guidance_model=None
    )
    ...
    # change reinitialize opt state to conduct finetuning
    from dataclasses import replace
    def update_state(state, new_opt_state):
        # Create a new instance with the updated opt_state
        return replace(state, opt_state=new_opt_state)
    # state.opt_state=optimizer.init(initial_params)
    state = update_state(state, optimizer.init(initial_params))
    ...
    for step in range(
        ...
        # Replicating a simple value (though usually not necessary for a boolean flag)
        # by creating an array with the value and replicating it
        classifier_eval_array = jnp.array([classifier_eval] * jax.local_device_count())
        replicated_classifier_eval = flax_utils.replicate(classifier_eval_array)
        
        # Execute one training step
        (_, pstate, replicated_classifier_eval), (ploss, p_log_stats) = p_train_step((next_rng, pstate, replicated_classifier_eval), batch)
        ...
        if step % config.training.eval_freq == 0:
            ...
            (_, _, _), (peval_loss, peval_log_stats) = p_eval_step(
                (next_rng, pstate, replicated_classifier_eval), eval_batch
            )
